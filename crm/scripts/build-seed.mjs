...
const STORIES = extract('var PRODUCT_STORIES = {', '{', '\n  };');
...
for (const p of PRODUCTS) {
  ...
  if (!STORIES[p.name]) throw new Error(`PRODUCT_STORIES has no entry for ${p.name}`);
}
...
PRODUCTS.forEach((p, i) => {
  ...
  emit(productRow({
    ...
    description: [STORIES[p.name], p.variants && `Colourways: ${p.variants.map((v) => v.label).join(', ')}.`]
      .filter(Boolean).join(' '),
